...
快连AI 仅作端点和消息的透传管理，不会记录、保存、审查聊天内容。聊天请求直接发至你配置的模型服务商，由其处理。

使用本工具须遵守所在地法律法规及所连接模型服务商的使用条款。本工具不对模型输出的任何内容负责。
...
Kuai Lian AI is a pass-through manager for endpoints and messages. It does not record, store, or review chat content. Chat requests are sent directly to the model providers you configure.

Users must comply with applicable laws and the terms of service of the model providers. This tool is not responsible for any content generated by the models.
