...
function resolveNodeConfig(nodeId) {
		...
		function copyOwnDataProperty(target, key, value) {
		...
		if (node.params) {
				for (var k in node.params) {
						if (Object.prototype.hasOwnProperty.call(node.params, k)) copyOwnDataProperty(config.params, k, node.params[k]);
				}
		}
		if (Object.prototype.hasOwnProperty.call(node, 'voice') && !Object.prototype.hasOwnProperty.call(config.params, 'voice')) copyOwnDataProperty(config.params, 'voice', node.voice);
		if (Object.prototype.hasOwnProperty.call(node, 'instruction') && !Object.prototype.hasOwnProperty.call(config.params, 'instruction')) copyOwnDataProperty(config.params, 'instruction', node.instruction);
		for (var i = ancestors.length - 1; i >= 0; i--) {
				var ancestor = ancestors[i];
				var ap = ancestor.params;
				if (ap) {
						for (var k in ap) {
								if (Object.prototype.hasOwnProperty.call(ap, k) && !Object.prototype.hasOwnProperty.call(config.params, k)) {
										copyOwnDataProperty(config.params, k, ap[k]);
								}
						}
				}
				if (!Object.prototype.hasOwnProperty.call(config.params, 'voice') && (!ap || !Object.prototype.hasOwnProperty.call(ap, 'voice')) && Object.prototype.hasOwnProperty.call(ancestor, 'voice')) copyOwnDataProperty(config.params, 'voice', ancestor.voice);
				if (!Object.prototype.hasOwnProperty.call(config.params, 'instruction') && (!ap || !Object.prototype.hasOwnProperty.call(ap, 'instruction')) && Object.prototype.hasOwnProperty.call(ancestor, 'instruction')) copyOwnDataProperty(config.params, 'instruction', ancestor.instruction);
		}
		if (!config.type) {
		...
		if (typeAliases[config.type]) config.type = typeAliases[config.type];
		if (!config.style) {
				config.style = "openai";
		}
		return config;
...
function detectModelType(name) {
		...
		if (lower.indexOf("whisper") >= 0 || lower.indexOf("transcrib") >= 0 || lower.indexOf("asr") >= 0) return "asr";
		if (lower.indexOf("tts") >= 0 || lower.indexOf("audio") >= 0 || lower.indexOf("speech") >= 0 || lower.indexOf("voice") >= 0) return "tts";
		if (lower.indexOf("image") >= 0 || lower.indexOf("dall-e") >= 0 || lower.indexOf("diffusion") >= 0 || lower.indexOf("flux") >= 0 || lower.indexOf("imagen") >= 0) return "image-generation";
		if (lower.indexOf("video") >= 0 || lower.indexOf("seedance") >= 0 || lower.indexOf("kling") >= 0) return "video-generation";
		return "chat";
}
...
function restoreObject(target, snapshot) {
		const restored = snapshotData(snapshot);
		for (const key of Object.keys(target)) delete target[key];
		for (const key of Object.keys(restored)) {
				setOwnEnumerableDataProperty(target, key, restored[key]);
		}
}
...
function restoreEndpoints(checkpoint) {
		...
		function restoreNode(nodeSnapshot) {
				...
				for (const key of Object.keys(nodeSnapshot)) {
						if (key !== 'children') setOwnEnumerableDataProperty(node, key, snapshotData(nodeSnapshot[key]));
				}
				if (Object.prototype.hasOwnProperty.call(nodeSnapshot, 'children')) {
						const restoredChildren = Array.isArray(nodeSnapshot.children) ? nodeSnapshot.children.map(restoreNode).filter(Boolean) : [];
						if (reference) {
		...
		for (const key of Object.keys(snapshot)) {
				setOwnEnumerableDataProperty(data, key, key === 'nodes' ? checkpoint.nodes : snapshotData(snapshot[key]));
		}
...
function persistEndpointsMutation(mutate) {
		...
		const queued = enqueueMutation(endpointsMutationQueue, async () => {
				if (clearInProgress) return null;
				const checkpoint = checkpointEndpoints(endpointsData || {
						nodes: []
				});
...
async function addNode(parentId, data) {
		...
		if (Object.prototype.hasOwnProperty.call(data, 'params')) {
				node.params = JSON.parse(JSON.stringify(data.params));
		}
...
async function updateNode(nodeId, updates) {
		...
		return persistEndpointsMutation(() => {
				...
				const sourceUpdates = updates == null ? {} : Object(updates);
				for (const key of Reflect.ownKeys(sourceUpdates)) {
						const descriptor = Object.getOwnPropertyDescriptor(sourceUpdates, key);
						if (!descriptor
								|| !descriptor.enumerable
								|| !Object.prototype.hasOwnProperty.call(descriptor, 'value')) continue;
						Object.defineProperty(normalizedUpdates, key, {
				...
				if (Object.prototype.hasOwnProperty.call(normalizedUpdates, 'params')) {
						normalizedUpdates.params = JSON.parse(JSON.stringify(normalizedUpdates.params));
				}
				const requestedRemoveFields = Object.prototype.hasOwnProperty.call(normalizedUpdates, '_removeLegacyParamFields')
						? normalizedUpdates._removeLegacyParamFields
						: undefined;
				delete normalizedUpdates._removeLegacyParamFields;
				const removableLegacyFields = new Set(['voice', 'instruction']);
				const removeFields = Array.isArray(requestedRemoveFields) ? requestedRemoveFields.filter(field => removableLegacyFields.has(field)) : [];
				const hasIsFullUrl = Object.prototype.hasOwnProperty.call(normalizedUpdates, 'isFullUrl');
				const hasDirectUrl = Object.prototype.hasOwnProperty.call(normalizedUpdates, 'directUrl');
				for (const key of Reflect.ownKeys(normalizedUpdates)) {
						const descriptor = Object.getOwnPropertyDescriptor(normalizedUpdates, key);
						if (!descriptor
								|| !descriptor.enumerable
								|| !Object.prototype.hasOwnProperty.call(descriptor, 'value')) continue;
				...
				if (hasIsFullUrl) {
						node.isFullUrl = !!normalizedUpdates.isFullUrl;
						delete node.directUrl;
				} else if (hasDirectUrl) {
						node.isFullUrl = !!normalizedUpdates.directUrl;
						delete node.directUrl;
				}
				for (const field of removeFields) delete node[field];
				return node;
		});
}
// 递归删除节点及其所有子代
		...
		return persistEndpointsMutation(() => {
				...
				function deepClone(source) {
						...
						if (Object.prototype.hasOwnProperty.call(source, 'params')) {
							cloned.params = snapshotData(source.params);
						}
		...
		for (const msg of session.messages) {
				if (msg.role === 'assistant' && msg.responses) {
						for (const r of msg.responses) {
								const m = {
										role: 'assistant',
										timestamp: r.timestamp || msg.timestamp || Date.now()
								};
						...
						if (copy.role === 'assistant') {
								...
								if (copy.endpointGroupId) {
										delete copy.endpointGroupId;
										changed = true;
								}
								if (copy.usage) {
										delete copy.usage;
